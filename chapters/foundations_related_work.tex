...
The capabilities of VERA are demonstrated by a mobile mini-robot called EMARO (Educational Modular Autonomous Robot Osnabrück), which was developed at Osnabrück University specifically for research and education in robotics and artificial intelligence \cite{Geh24}.

The modular hardware architecture of EMARO is based on three main PCBs: a \\textbf{Host-PCB} containing high-level computing, a \\textbf{Power-PCB} managing energy supply and motor control and a \\textbf{Base-PCB} integrating ground sensors \cite{Geh24}. The specific variant used in this work is equipped with a Raspberry Pi Compute Module 4 (CM4) running Ubuntu and the Robot Operating System 2 (ROS~2) \cite{Geh24}.
...
For environmental perception, the robot is equipped with Time-of-Flight (ToF) distance sensors and an Inertial Measurement Unit (IMU) for orientation tracking \cite{Geh24}. Moreover, there are two wide-angle cameras that can be configured for stereo vision applications or line-following tasks \cite{Geh24}. Within VERA, EMARO takes on the role of a "Physical Twin," streaming real-time telemetry including odometry, battery status, and sensor data to the simulation framework via ROS~2 topics \cite{Geh24}.

\subsection{Original VERA System Architecture}
The software architecure of the original VERA framework was mainly a ROS~2 implementation, and its structure included only three main components: the Virtual Environment Positioning System (VEPS), the Environment Manager, and the Visualization System \cite{Geh24}.
\subsubsection{Virtual Environment Positioning System (VEPS)}
Accurate localization is important to synchronize the physical robot with the projected virtual world \cite{Geh24}. For this purpose, the original implementation of VERA relied on a depthbased approach where a ceiling-mounted Allied Vision Ruby 3D stereo camera captured point clouds of the test area \cite{Geh24}. The point clouds were filtered by a Median Absolute Deviation (MAD) algorithm to calculate the centroid of the robot \cite{Geh24}.
However, for maximum robustness and precision for this thesis, the positioning system was updated to use an **ArUco marker-based tracking system** developed in parallel research by \cite{JuliaBA}. Although the point-cloud method provided a markerless solution, the ArUco-based approach has higher reliability and lower noise under changing light conditions  \cite{JuliaBA,Geh24}. By attaching a fiducial marker to the top of the EMARO robot, the system is able to calculate the 6D pose (position and orientation) of the robot \cite{JuliaBA}. This pose is published into the ROS~2 network, where it can instantly update the position of the digital twin \cite{JuliaBA}.
...
The visualization component is responsible for rendering the virtual environment so it can be projected onto the physical floor \cite{Geh24}. In the original VERA framework, this was achieved using a custom ROS~2 node built with **Pygame**, a Python library designed for 2D game development \cite{Geh24}.
...
\begin{itemize}
    \item \textbf{Lack of Realistic Physics:} The custom Environment Manager does not feature a physics engine. Collision detection is performed through simple radius checks, meaning the robot cannot push objects or experience friction nor can it interact with complex geometries. This reduces the realism of the Digital Twin \cite{Geh24}.
...
The selelction of a capable simulation engine is an important part in overcoming the limitations of the original VERA framework. The choice of platform influences the efficiency, accuracy, and applicability of the Digital Twin \cite{Singh2025}. Based on this, the evaluation of simulation engines in this thesis is guided by four criteria which are vital for a functional Mixed Reality Digital Twin:

\begin{itemize}
    \item \textbf{Visual Fidelity:} It is not only about rendering quality but to support realistic sensor simulation, such as cameras, and to provide an immersive experience to the user in VR that lacks in traditional simulators \cite{dosSantos2025}.
    \item \textbf{Physics Accuracy:} The engine must provide a robust physics backend to accurately predict the kinematic and dynamic behavior of the EMAROS robot, a capability missing in the original custom manager \cite{Singh2025}.
    ...
    \item \textbf{Robust ROS~2 Integration:} ROS~2 forms the data backbone of the system and, hence, should enable low-latency, high-throughput communication to synchronize the digital twin with the physical robot in real-time \cite{Singh2025}.
\end{itemize}
...
Game engines have been widely adopted within the robotics community to overcome the visualization limitations of traditional simulators.
...
\textbf{Unity Engine} has emerged as the dominating platform for Industry 5.0 applications, especially those focusing on HRI and MR \cite{Gonzalez2025}. It offers a balance of high-fidelity graphics with a robust physics engine, PhysX \cite{Singh2025}. Unity is also noted for having one of the most mature XR ecosystems, along with a huge asset store, lowering the barrier to entry for developers\cite{Singh2025, Coronado2023}. Literature highlights that the  user-friendly interface of Unity and supportive community help with a smoother learning curve compared to Gazebo or Unreal, particularly for users without deep C++ expertise \cite{Singh2025, Coronado2023}.
...
Based on this analysis, Unity is selected as the simulation engine. It has an integrated XR framework for Mixed Reality \cite{Coronado2023} and better visualization compared to Gazebo \cite{Gonzalez2025}. It strikes a balance between accessibility and performance, while C\\# scripting, along with large community support, makes learning it simpler than Unreal Engine \cite{Coronado2023}. In addition the \texttt{ros2-for-unity} plugin provides high-performance and low-latency communication \cite{Rob24}.

\section{Motivation}
\label{sec:motivation}

While the original framework of VERA was able to proof the concept of Robot-in-the-Loop testing, it has severe technical limitations due to its reliance on custom software components \cite{Geh24}. The lack of a physics engine prevented realistic interaction, the Pygame-based visualizer suffered from performance bottlenecks in complex scenes and Virtual Reality is not supported, limiting the potential for human-robot collaboration \cite{Geh24}.

To overcome these limitations, this thesis proposes integrated the \textbf{Unity Engine} \cite{Uni23} into the architecture of VERA using the \textbf{Robot Operating System 2 (ROS~2)} \cite{macenski2023survey}. Unity is chosen because it offers advanced physics simulation through PhysX, sophisticated rendering and a mature framework framework for creation in XR \cite{Singh2025, Gonzalez2025}. All of these target the shortcomings of the past system \cite{Geh24}. Using the \\texttt{ros2-for-unity} asset ensures low-latency, high-throughput synchronization between the physical and virtual world \cite{Rob24}.

The primary motivation is to establish a robust, physics-enabled Mixed Reality environment that bridges the gap between simulation and reality. The new development will be verified by creating demonstration scenarios. These scenarios will include the EMAROS robot actively interacting with virtual elements, acting as a proof-of-concept for the improved physical realism of the system, scalability, and immersive interactive capabilities.