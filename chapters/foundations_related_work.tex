...
\begin{figure}[h]
    ...
    \caption{The physical setup of the VERA platform, featuring a ceiling-mounted projector and tracking system above the test area. \cite{Geh24}}
    \label{fig:vera_setup}
\end{figure}

\subsection{The EMARO Test Platform}
\label{sec:emaro_platform}
The capabilities of the framework are demonstrated using the EMARO (Educational Modular Autonomous Robot Osnabrück), a mobile mini-robot developed at Osnabrück University specifically for research and education in robotics and artificial intelligence \cite{Geh24}.

EMARO is designed with a modular hardware architecture based on three main Printed Circuit Boards (PCBs): a \textbf{Host-PCB} for high-level computing, a \textbf{Power-PCB} for energy management and motor control, and a \textbf{Base-PCB} for integrating ground sensors. The specific variant used in this work is equipped with a Raspberry Pi Compute Module 4 (CM4) running Ubuntu and the Robot Operating System 2 (ROS~2) \cite{Geh24}.

\begin{figure}[h]
    \centering
    % TODO: Insert Figure 3.1 from Gehricke's Thesis (The EMARO robot)
    % \includegraphics[width=0.5\textwidth]{images/emaro_robot_placeholder.png}
    \caption{The EMARO robot serves as the physical twin in the system, equipped with a modular sensor suite and running ROS~2. \cite{Geh24}}
    \label{fig:emaro_robot}
\end{figure}

For environmental perception, the robot utilizes Time-of-Flight (ToF) distance sensors and an Inertial Measurement Unit (IMU) for orientation tracking. It also features two wide-angle cameras configured for stereo vision applications or line-following tasks. In the context of VERA, EMARO acts as the "Physical Twin," sending real-time telemetry (such as odometry, battery status, and sensor data) to the simulation framework via ROS~2 topics \cite{Geh24}.

\subsection{Original VERA System Architecture}
The software architecture of the original VERA framework was built entirely on ROS~2 and consisted of three main components: the Virtual Environment Positioning System (VEPS), the Environment Manager, and the Visualization System.

\subsubsection{Virtual Environment Positioning System (VEPS)}
Precise localization is critical for synchronizing the physical robot with the projected virtual world. The original VERA implementation utilized a depth-based approach, employing a ceiling-mounted Allied Vision Ruby 3D stereo camera to capture point clouds of the test area. A Median Absolute Deviation (MAD) algorithm filtered these point clouds to calculate the robot's centroid \cite{Geh24}.

However, to ensure maximum robustness and precision for this thesis, the positioning system has been updated to use an **ArUco marker-based tracking system**, developed in parallel research by \cite{JuliaBA}. While the point-cloud method provided a markerless solution, the ArUco-based approach offers significantly higher reliability and lower noise in variable lighting conditions. By placing a fiducial marker on top of the EMARO robot, the system can determine the robot's 6D pose (position and orientation) with sub-millimeter precision. This pose is published to the ROS~2 network, allowing the simulation to update the position of the digital twin instantly.
...
\begin{figure}[h]
    ...
    \caption{Visualization of the tracking system. The original VERA used point clouds (left), while the current iteration utilizes ArUco markers (right) for robust pose estimation \cite{JuliaBA}.}
    \label{fig:veps_comparison}
\end{figure}

\subsubsection{Virtual Environment Manager}
The Environment Manager acts as the central "brain" of the simulation. Implemented as a C++ ROS~2 node, it is responsible for parsing environment definitions stored in standard Gazebo \texttt{world.sdf} files. It manages the state of all virtual objects, tracking their positions and properties (e.g., whether an object is "movable" or "static") \cite{Geh24}.

A key function of the manager is handling interactions between the robot and the virtual world. It runs a continuous loop (typically at 20Hz) that checks the distance between the robot's position (provided by VEPS) and virtual objects. If a collision is detected, the manager triggers predefined actions, such as removing a "collectible" object or stopping the robot. However, this manager relies on simple distance heuristics rather than a physics engine, limiting the complexity of interactions \cite{Geh24}.

\subsubsection{Visualization System}
The visualization component is responsible for rendering the virtual environment so it can be projected onto the physical floor. In the original VERA framework, this was achieved using a custom ROS~2 node built with **Pygame**, a Python library designed for 2D game development \cite{Geh24}.

The visualizer subscribes to the object lists published by the Manager and the robot's path history. It renders a 2D top-down orthogonal view of the scene, drawing obstacles as simple geometric shapes. Additionally, it provides Augmented Reality (AR) features by projecting dynamic information directly into the workspace, such as the robot's driven path (visualized as a trail) and a system status panel displaying real-time CPU and battery usage behind the robot \cite{Geh24}.
...
\begin{figure}[h]
    ...
    \caption{The original VERA visualization rendered via Pygame, projecting the robot's path and virtual obstacles onto the floor. \cite{Geh24}}
    \label{fig:pygame_vis}
\end{figure}

\subsection{Identified Limitations}
While VERA successfully demonstrated the concept of Robot-in-the-Loop testing with AR projections, its original software implementation presents several critical limitations that this thesis aims to overcome:

\begin{itemize}
    \item \textbf{Lack of Realistic Physics:} The custom Environment Manager does not feature a physics engine. Collisions are detected using simple radius checks, meaning the robot cannot push objects, experience friction, or interact with complex geometries. This limits the realism of the Digital Twin \cite{Geh24}.
    \item \textbf{Performance Scalability:} The Pygame-based visualizer processes rendering on the CPU and struggled with performance in complex scenarios. Gehricke \cite{Geh24} noted that in environments with a high number of dynamic objects (e.g., $>$800), the message queue became saturated, leading to significant latency and incomplete visual updates.
    \item \textbf{Limited Interaction (No VR):} The system was designed strictly for 2D floor projections. There is no support for immersive Virtual Reality (VR) interfaces that would allow a human operator to view the scene in 3D or interact with the robot remotely. The integration of VR was explicitly identified as a necessary future step to enhance Human-Robot Interaction (HRI) \cite{Geh24}.
\end{itemize}

\section{Simulation Engines for Robotics}
\label{sec:sim_engines}

To overcome the limitations of the original VERA framework—specifically the lack of physical realism and the inability to support immersive VR interaction—selecting a capable simulation engine is critical. The choice of platform significantly influences the efficiency, accuracy, and applicability of the Digital Twin \cite{Singh2025}. Therefore, the evaluation of simulation engines in this thesis is driven by four specific technical criteria essential for a functional Mixed Reality Digital Twin:

\begin{itemize}
    \item \textbf{Visual Fidelity:} High-quality rendering is required not just for aesthetics, but to support realistic sensor simulation (e.g., cameras) and to provide an immersive experience for the user in VR, which traditional simulators often lack \cite{dosSantos2025}.
    \item \textbf{Physics Accuracy:} The engine must provide a robust physics backend to accurately predict the kinematic and dynamic behavior of the EMAROS robot, a capability missing in VERA's original custom manager \cite{Singh2025}.
    \item \textbf{Native VR/AR Support:} To extend VERA with the proposed Virtual Reality interfaces, the engine must have a mature framework for XR devices, facilitating the creation of interactive environments without extensive middleware \cite{Coronado2023}.
    \item \textbf{Robust ROS~2 Integration:} As ROS~2 serves as the data backbone of the system, the engine must support low-latency, high-throughput communication to synchronize the digital twin with the physical robot in real-time \cite{Singh2025}.
\end{itemize}
...
While the original VERA framework successfully demonstrated the concept of Robot-in-the-Loop testing, its reliance on custom software components imposed significant technical limitations. The lack of a physics engine prevented realistic interaction, the Pygame-based visualizer suffered from performance bottlenecks in complex scenes, and the absence of Virtual Reality support limited the potential for immersive human-robot collaboration \cite{Geh24}.

To overcome these constraints, this thesis proposes re-engineering the core architecture of VERA by integrating the \textbf{Unity Engine} with the \textbf{Robot Operating System 2 (ROS~2)}. Unity was selected for its robust physics simulation (PhysX), high-fidelity rendering, and mature XR framework, which directly address the shortcomings of the previous system \cite{Singh2025, Gonzalez2025}. By utilizing the \texttt{ros2-for-unity} asset, the system ensures high-performance, low-latency synchronization between the physical and virtual worlds \cite{Rob24}.

The primary motivation is to establish a robust, physics-enabled Mixed Reality environment that bridges the gap between simulation and reality. This new development will be validated through the creation of specific \textbf{demonstration scenarios}. These scenarios will feature the EMAROS robot actively interacting with virtual elements, serving as a proof-of-concept for the system's enhanced physical realism, scalability, and immersive interactive capabilities.