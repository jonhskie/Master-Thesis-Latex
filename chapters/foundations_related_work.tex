...
The capabilities of VERA are demonstrated by a mobile mini-robot called EMAROs (Educational Modular Autonomous Robot Osnabrück), shown in Figure~\ref{fig:EMAROs_robot}, which was developed at Osnabrück University specifically for research and education in robotics and artificial intelligence~\cite{Geh24}.
The modular hardware architecture of EMAROs is based on three main PCBs: a \textbf{Host Printed Circuit Board (PCB)} containing high-level computing, a \textbf{Power-PCB} managing energy supply and motor control and a \textbf{Base-PCB} integrating ground sensors~\cite{Geh24}. The specific variant used in this work is equipped with a Raspberry Pi Compute Module 4 (CM4) running Ubuntu~24.04 and the Robot Operating System 2 (ROS~2)~\cite{Geh24}.
