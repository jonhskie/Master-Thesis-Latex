...
\begin{itemize}
    ...
    \item \textbf{FR-02: Physics Simulation:}\label{req:FR-02} Overcoming the limitations in the previous VERA framework, the engine shall provide a physics system that can resolve mass, friction, collision, and drag to simulate physical interactions such as a robot pushing obstacles.
...
\begin{itemize}
    ...
    \item \textbf{FR-08: Tracking Failsafe:}\label{req:FR-08} The system shall provide a failsafe that halts the movement of the robot in case of an interruption of the tracking data stream beyond a defined threshold to avoid unwanted movements due to signal loss.
...
\begin{itemize}
    ...
    \item \textbf{FR-11: Sensor Simulation:}\label{req:FR-11} The system shall provide simulated sensors able to generate data, including video feeds and ranging data. The sensors shall be capable of detecting both static environment structures and dynamic obstacles.
    
    \item \textbf{FR-12: Command Interface:}\label{req:FR-12} The system shall provide a generic network interface to receive commands and return status feedback to enable the robot to trigger simulation events.
    
    \item \textbf{FR-13: Dynamic Environment Interaction:}\label{req:FR-13} The system shall allow dynamic manipulation of the environment, including physical pushing and attaching of objects, modification of the state of objects, and changes in surface textures at runtime.
...
\begin{itemize}
    ...
    \item \textbf{FR-17: Goal Setting:}\label{req:FR-17} The interface shall provide the user with the functionality to set navigation targets compatible with the Nav2 navigation stack by pointing at the virtual ground using the mouse or the VR controller.
...
\begin{itemize}
    ...
    \item \textbf{NFR-02: Frame Rate:}\label{req:NFR-02} The simulation shall maintain a constant target frame rate (at least 60 frames per second (FPS)) to avoid simulator sickness in VR~\cite{Chang20102020, Yang2019} and to ensure that AR projections remain visually stable and aligned with the physical environment.    
