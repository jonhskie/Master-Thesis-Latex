...
In addition, the system simulates sensors for the robot. It generates virtual LiDAR scans and camera images from the robot's perspective, allowing robotic applications to receive virtual sensor data. This enables thorough testing of navigation and vision algorithms within the Mixed Reality Environment.
...
Figure~\ref{fig:software_architecture} provides an overview of how these Unity scripts interact with the external ROS~2 nodes via topics. Detailed descriptions of these components follow in Sections~\ref{sec:robot_representation_and_control} and~\ref{sec:environmental_simulation_capabilities}.
...
\label{subsec:sensor_simulation}
...
\label{subsec:sensor_data_projection}
...
\label{subsec:interactive_control_interfaces}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/logistics_debug.png}  % Make sure extension is .png or .jpg
        \par\vspace{5pt} % Small gap between image and text
        (a)
    \end{minipage}%
    \hfill % Spacer
    ...
    \begin{minipage}{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/logistics_debug_1.png} % Make sure extension is .png or .jpg
        \par\vspace{5pt}
        (b)
    \end{minipage}
    \caption{Example debug views of the Logistics application. (a) The processor identifies a blue transport box in its ROI (green) and calculates the centroid (red dot). (b) The system found a blue delivery zone and verifies its position.}
...
This application represents the most complex use case, operating in a simulated farm with an agricultural field. The goal is to identify the field boundaries and position and sequentially apply three tools: a plow, a seeder, and a harvester to the field.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/debug_view_edge_following_1.png}   % Make sure extension is .png or .jpg
        \par\vspace{5pt} % Small gap between image and text
        (a)
    \end{minipage}%
    \hfill % Spacer
    ...
    \begin{minipage}{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/debug_view_edge_following.png}  % Make sure extension is .png or .jpg
        \par\vspace{5pt}
        (b)
    \end{minipage}
    \caption{Debug views of the Smart Farming application. (a) The processor identifies the boundary between the field and the white floor. (b) The robot follows the edge. The green line represents the detected edge used for navigation, while the red dot indicates the centroid used to compute the lateral and angular errors displayed in the overlay.}
...
With the software architecture defined and the functional modules integrated, the system is prepared for comprehensive analysis. The following chapter addresses the qualitative validation of the Mixed Reality interfaces and scenario realizations, followed by a quantitative assessment of performance metrics such as system latency and resource utilization to determine the operational limits of the platform.