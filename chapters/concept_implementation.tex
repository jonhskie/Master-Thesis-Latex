...
To ensure high performance and support for specific hardware peripherals, the system topology is consolidated into a single high-performance workstation acting as the central server, supported by distributed wireless clients. As shown in Figure \ref{fig:hardware_topology}, the architecture is divided into four distinct computational domains:

\begin{enumerate}
    \item \textbf{Workstation (Windows 11 Host):} The primary host runs Windows 11 to support the Unity Engine and the \textbf{ALVR (Air Light VR) Server}~\cite{ALVR_Github} natively. This layer manages the physics engine (PhysX) and renders the digital twin. It physically interfaces with the visual peripherals: a ceiling-mounted \textbf{Projector} via HDMI for floor projection and an \textbf{Overhead Tracking Camera} via USB to provide localization data.
    
    \item \textbf{Control Environment (WSL 2):} The computationally intensive robotic software runs within the Windows Subsystem for Linux (Ubuntu 24.04) on the same workstation. This isolation allows the \textit{Nav2 Stack}, \textit{Agent Mission Logic}, and the \textit{ArUco Tracking System} to run in a native Linux environment while communicating with the Windows host via a low-latency \textbf{Localhost} connection.
    ...
    \item \textbf{VR Interface (Meta Quest Pro):} The VR headset functions as a standalone thin client running \textbf{Meta Horizon OS}. It executes the \textbf{ALVR Client}~\cite{ALVR_Github} application, which is responsible for decoding the incoming video stream and handling spatial input.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{images/hardware_topology.png}
    \caption{Hardware and Network Topology. The workstation acts as the central computation unit, hosting the Unity simulation on Windows and the autonomous navigation stack within WSL. The physical robot and VR headset communicate via Wi-Fi through a central router, while visual peripherals interface directly with the Windows host.}
    \label{fig:hardware_topology}
\end{figure}

The VR integration (\hyperref[req:FR-16]{FR-16}) relies on ALVR, an open-source remote rendering solution that splits the computational workload. The \textbf{ALVR Server}, running on the high-performance Windows workstation, captures the rendered frames from Unity and encodes them into a high-bitrate video stream. This stream is transmitted via Wi-Fi to the \textbf{ALVR Client} on the Meta Quest Pro. The headset decodes and displays this video feed while simultaneously capturing 6-DoF (Six Degrees of Freedom) head-tracking data and controller inputs, which are transmitted back to the server in real-time.~\cite{ALVR_Github}

The network infrastructure is centered around a dedicated \textbf{Router}. The workstation is connected via \textbf{Ethernet} to ensure a stable high-bandwidth connection for the simulation server, while the EMAROs robot and Meta Quest Pro connect via \textbf{Wi-Fi}. This topology ensures that latency-sensitive traffic, such as the ALVR video stream and ROS~2 control commands, faces minimal interference.
\subsection{Software Component Architecture}
\label{subsec:software_components}

The software architecture follows a modular publisher-subscriber pattern. To integrate the Unity game engine with the robotic network, the system utilizes the \texttt{ROS2ForUnity} library (\hyperref[req:FR-04]{FR-04}) \cite{ROS2ForUnityDocs}.

To facilitate modularity, the implementation relies on Unity's component-based architecture. In this framework, entities within the simulation, such as the robot or the environment, are represented as \texttt{GameObjects}. These objects function as containers that hold various functional modules known as \textit{components}. By inheriting from Unity's base class \texttt{MonoBehaviour}, C\# scripts become components that can be attached to these \texttt{GameObjects} to define their behavior.
...
The functionality of the digital twin is driven by such scripts that interact with the engine's internal state machine. To ensure that physical simulation remains deterministic while visualization remains smooth, the architecture uses specific phases of the Unity execution loop~\cite{UnityDocumentation}:
