...
\begin{enumerate}
    \item \textbf{Simulation Host (Windows 11):} The Unity engine runs natively on Windows. This operating system was selected because the Unity Editor's VR interfaces are not currently supported on Linux. This layer handles the physics engine (PhysX), renders the digital twin, and interfaces with the VR headset. Additionally, a projector is connected to this host to visualize the environment on the physical floor \cite{PLACEHOLDER_HARDWARE_SETUP}.
    
    \item \textbf{Control Environment (WSL/Ubuntu):} ROS~2 runs within the Windows Subsystem for Linux (WSL, Ubuntu 24.04). This environment hosts the computationally intensive software, such as the autonomous navigation stack (\textit{Nav2}) and the mission logic agents. Running these components directly on the robot causes severe performance issues or system crashes due to limited onboard resources, so they are offloaded to the workstation to ensure stable and responsive operation.
...
For the Mixed Reality interface \textbf{(FR-16)}, the visual output from Unity is streamed via Wi-Fi to the Meta Quest Pro headset using \textit{ALVR}, an open-source streaming solution. This setup allows the user to move freely without a cable tether.
...
The core of this integration is the \texttt{ROS2UnityComponent} class. This component acts as a central wrapper around the standard ROS 2 middleware. Inheriting from Unity's \texttt{MonoBehaviour} class, it must be attached to a \texttt{GameObject} in the scene to function. When the application starts, this component initializes the ROS 2 context, verifies that the connection is valid, and manages the lifecycle of the simulation's ROS~2 nodes. It allows other scripts to access the ROS network by referencing this central component to create publishers, subscribers, and service clients.

\textbf{Unity Scripting and Execution Loops}

The functionality of the digital twin is governed by C\# scripts that interact with the engine's internal state machine. To ensure that physical simulation remains deterministic while visualization remains smooth, the architecture exploits specific phases of the Unity execution loop:

\begin{itemize}
    \item \textbf{Initialization (\texttt{Start}):} This method is called exactly once when the script is first enabled, before any frames are updated. In this architecture, it is used to retrieve the reference to the \texttt{ROS2UnityComponent}, initialize the specific ROS nodes, and set up the necessary publishers and subscribers.
...
\begin{figure}[ht]
    ...
    \caption{Software Component Architecture. Unity scripts act as ROS nodes, publishing sensor data and subscribing to control commands. The external control stack, running in WSL, processes this data to make autonomous decisions.}
...
To ensure consistency, a \texttt{RosTimeHelper.cs} utility is used by all simulated sensors. This script ensures that every piece of sensor data leaving Unity is stamped with the exact simulation time of the frame it was rendered in, preventing data drift.
...
\begin{enumerate}
    \item \textbf{Shutdown:} The active ROS 2 connection is terminated. This ensures that all Unity nodes are properly removed from the network, preventing any lingering nodes from publishing outdated or invalid data.
...
%%%%%%%%%%%%%%%%%%%%%FERTIG%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%ABER NOCH NICHT DE AIed%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{enumerate}
    \item \textbf{FindEquipment:} The robot executes a \textbf{Spiral Search} pattern using the Nav2 stack to locate a specific tool.
...
\begin{enumerate}
    \item \textbf{FindBox (Spiral Search):} The robot executes the \texttt{find\_box\_action\_server}. This generates a spiral pattern of waypoints using the Nav2 stack to efficiently cover the floor plan. The robot navigates these waypoints until the \texttt{BoxDetectionProcessor} identifies a valid target in the camera's Region of Interest.
...
\textbf{State Machine Logic}
While the previous agents relied on the Nav2 stack for motion planning, this agent implements a custom \textbf{Visual Servoing} control loop. The \texttt{mission\_state\_machine.py} orchestrates a three-stage sequence:
...
\begin{enumerate}
    ...
    \item \textbf{FollowLine:} The robot engages a custom Proportional-Integral-Derivative (PID) controller implemented in \texttt{follow\_line\_action\_server.py}. This controller calculates velocity commands (\texttt{cmd\_vel}) directly from the perception error values. By bypassing the global path planning and costmap layers of Nav2, this direct control loop achieves the low latency required to react to sudden changes in the user-drawn line. If the line is erased or ends, the state machine transitions back to \texttt{FindLine} to search for a new path.
\end{enumerate}
